...
const Recomendations = ({ rec, animes, filtered, req, anime, genres, isSearchMode }) => {
  return rec ? (
    <div className="absolute top-16 left-0 z-50 w-full bg-third rounded-lg shadow-lg">
      ...
      <div className="p-4 border-b border-second/20">
        <p className="text-base font-semibold text-second/70">
          {anime !== "" ? `Hasil pencarian "${anime}"` : "Rekomendasi"}
        </p>
      </div>

      {/* Content */}
      <div className="flex flex-col lg:flex-row gap-6 p-2 lg:p-4">
        {/* Genre List - Hanya tampil saat bukan search mode */}
        {!isSearchMode && (
          <div className="lg:w-1/4 p-2">
            ...
            <Link
              to="/genres"
              className="hidden lg:flex items-center gap-2 text-sm font-medium text-main hover:underline mt-2"
            >
              lainnya
              <img
                src={"/arrow.svg"}
        ...
        {/* Anime List */}
        <div className={`flex flex-col gap-6 pr-2 ${isSearchMode ? 'w-full' : 'md:w-3/4'} overflow-y-scroll eps max-h-80 h-full`}>
          <h3 className="text-sm font-bold text-second">
            {isSearchMode ? `Ditemukan ${animes.length} anime` : 'Anime'}
          </h3>
          
          {/* Loading State */}
          {animes.length === 0 && !isSearchMode && req === "type=ongoing" ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              {(isSearchMode ? animes : filtered.slice(0, 10)).map((anime, index) => (
                <Link
                  to={`/anime/${anime.slug}`}
                  key={index}
                  className="relative block overflow-hidden rounded-lg transition group"
                  aria-label={`Anime ${anime.judul}`}
                >
                  <img
                    src={anime.gambar || '/placeholder.jpg'}
